...
@section('contents')
    ...
    <a href="{{route('author.post.index')}}" class="btn btn-sm btn-primary" style="margin-bottom: 9px;" title="back to post page">Back</a>
    <a href="{{route('author.post.edit', $post->id )}}" class="btn btn-sm btn-info" style="margin-bottom: 9px;" title="want to edit  this post">Edit </a>


    @if($post->is_approved ==false)
     <button type="button" style="margin-bottom: 9px;" title="{{$post->user->name}} your post is waiting for approving " class="btn pull-right btn-sm btn-danger">
        <span class="glyphicon glyphicon-warning-sign"> </span> not approve
     </button>
    @else
        <button type="button" style="margin-bottom: 9px;" title="{{$post->user->name}} your post is already approved "  class="btn pull-right btn-sm btn-success">
